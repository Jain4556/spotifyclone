...
// Example usage:
const totalSeconds = 75; // Replace with your desired number of seconds
...
console.log(formattedTime); // Output: 01:15
 ...
 async function  getSongs(folder) {
    currFolder = folder;
     let a = await fetch(`http://192.168.128.86:5500/${folder}/`)
...
async function displayAlbums(){
    let a = await fetch(`http://192.168.128.86:5500/songs/`)
     let response = await a.text();
     let div = document.createElement("div")
     div.innerHTML = response;
   ...
   for (let index = 0; index < array.length; index++) {
    ...
    let a = await fetch(`http://192.168.128.86:5500/songs/${folder}/info.json`)
...
// Add an event listener to  next 
    ...
    document.querySelector(".range").getElementsByTagName("input")[0].addEventListener("change", (e)=>{
        console.log("Setting volujme to", e.target, e.target.value, "/100")
